...
impl Instruction {
	pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
		...
		let instruction = match instruction {
			...
			RawInstruction(OperationCode::LoadConstant, Layout::ABx { a, bx }) => Self::LoadConstant {
				destination: Register(a),
				source: Constant(bx),
			},
			RawInstruction(OperationCode::LoadBoolean, Layout::ABC { a, b, c }) => Self::LoadBoolean {
				destination: Register(a),
				value: b == 1,
				skip_next: c == 1,
			},
			RawInstruction(OperationCode::LoadNil, Layout::ABC { a, b, .. }) => Self::LoadNil(
				Range {
					start: Register(a),
					end: Register(b),
				}
			),
			RawInstruction(OperationCode::GetUpvalue, Layout::ABC { a, b, .. }) => Self::GetUpvalue {
				destination: Register(a),
				upvalue: Upvalue(b),
			},
			...
			RawInstruction(OperationCode::Concatenate, Layout::ABC { a, b, c }) => Self::Concatenate {
				destination: Register(a),
				operands: Range {
					start: Register(b),
					end: Register(c),
			...
			RawInstruction(OperationCode::LessThanOrEqual, Layout::ABC { a, b, c }) => Self::LessThanOrEqual {
				lhs: RegisterOrConstant::from(b as u32),
				rhs: RegisterOrConstant::from(c as u32),
				comparison_value: a == 1,
			},
			...
			RawInstruction(OperationCode::Return, Layout::ABC { a, b, .. }) => Self::Return(
				Range {
					start: Register(a),
					end: Register(b),
				}
			),
			RawInstruction(OperationCode::IterateNumericForLoop, Layout::AsBx { a, sbx }) => Self::IterateNumericForLoop {
				initial_value: Register(a),
				step: sbx,
			},
			RawInstruction(OperationCode::PrepareNumericForLoop, Layout::AsBx { a, sbx }) => Self::PrepareNumericForLoop {
				internal_index: Register(a),
				step: sbx,
			},
			RawInstruction(OperationCode::IterateGenericForLoop, Layout::ABC { a, c, .. }) => Self::IterateGenericForLoop {
				iterator: Register(a),
			...
			RawInstruction(OperationCode::VarArg, Layout::ABC { a, b, .. }) => Self::VarArg(
				Range {
					start: Register(a),
					end: Register(b),
				}
			),
			_ => return Err(Err::Failure(Error::from_error_kind(
				input,
				ErrorKind::Switch,
			))),
		};
